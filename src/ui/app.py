...
from utils.logging import reformat
    ...
    def run(self):
        with gr.Blocks(title="The Price is Right", fill_width=True) as ui:
            ...
            def get_plot():
                ...
                # Handle empty or insufficient vector DB (e.g. fresh local run)
                if vectors.size == 0:
                    fig = go.Figure()
                    fig.update_layout(
                        title="No products in vector DB yet. Run the scanner to ingest products (need >30 for 3D plot).",
                        height=400,
                    )
                    return fig

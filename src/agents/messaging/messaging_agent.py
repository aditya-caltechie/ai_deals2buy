...
class MessagingAgent(Agent):
    ...
    def __init__(self):
        ...
        self.pushover = PushoverClient(user=self.pushover_user, token=self.pushover_token)
    ...
    def push(self, text):
        ...
        self.pushover.send(text, sound="cashregister")
    ...
    def craft_message(self, description: str, deal_price: float, estimated_true_value: float) -> str:
        user_prompt = (
            "Please summarize this great deal in 2-3 sentences to be sent as an exciting push "
            "notification alerting the user about this deal.\n"
        )
        user_prompt += (
